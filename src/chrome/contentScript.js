  ...
  window.addEventListener('load', function() {
    ...
    for(var i = 0; i < links.length; i++) {
      ...
      if(links[i].href.indexOf('open-eid-sign:') == 0) {
        links[i].setAttribute('onmouseup', 'window.openEID_link = this; window.openEID.sign(function(json) { window.open(window.openEID_link.href.substring(14) + \'#\' + encodeURIComponent(JSON.stringify(json))); window.openEID_link = null; });');
        links[i].setAttribute('onclick', 'return false;');
      }      
    }
  });
